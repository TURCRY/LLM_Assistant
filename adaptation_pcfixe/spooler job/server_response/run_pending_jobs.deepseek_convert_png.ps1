param(
    ...
    [string]$DeepSeekConvertScript = "D:\GPT4all_local\proposed_changes\spooler job\server_response\run_deepseek_ocr_job.convert_png.ps1",
    [string]$DeepSeekPython = "D:\GPT4all_local\.venv\Scripts\python.exe",
...
function Build-DeepSeekOcrConvertCommand {
    param(
        [object]$Job,
        [string]$RunningJobPath
    )
    ...
    $outputDir = Assert-AllowedPath -PathValue (Get-JobString -Job $Job -Name "output_dir") -FieldName "output_dir"
    $pngDir = Assert-AllowedPath -PathValue (Get-JobString -Job $Job -Name "png_dir") -FieldName "png_dir"
    $script = Assert-AllowedPath -PathValue $DeepSeekConvertScript -FieldName "DeepSeekConvertScript"
    $python = Assert-AllowedPath -PathValue $DeepSeekPython -FieldName "DeepSeekPython"

    if ([IO.Path]::GetExtension($sourcePdf) -ne ".pdf") { throw "source_pdf doit etre un PDF: $sourcePdf" }
    if (-not (Test-Path -LiteralPath $sourcePdf)) { throw "PDF source introuvable: $sourcePdf" }
    if (-not (Test-Path -LiteralPath $script)) { throw "Helper DeepSeekOCR introuvable: $script" }
    if (-not (Test-Path -LiteralPath $python)) { throw "Python venv introuvable: $python" }
    New-Item -ItemType Directory -Path $pngDir -Force | Out-Null

    [void](Assert-Pages -Value (Get-JobValue -Job $Job -Name "pages"))
...
try {
    ...
    try {
        switch ([string]$job.type) {
            ...
            "deepseek_ocr" {
                $cmd = Build-DeepSeekOcrConvertCommand -Job $job -RunningJobPath $runningJobPath
            }
            default {
